...
//	    System.out.println("3. 분야 별 도서 목록..?");
		System.out.println("4. 뒤로 가기");
		System.out.println("----------------------------------");
	...
	public void BestSeller() {
		// 1. 대출횟수 순 2. 평점순
		System.out.println("_______[ 배스트셀러 ] _______");
	...
	public void RentalBook() {
		...
		if (id != null) {
			...
			if (list != null) {
				...
				System.out.print("대여하실 책의 연번을 입력해주세요 : ");
				...
				if (stock >= 1) {
					...
					// 대출 후 로그인후의 화면으로 이동
					// iltest.InsertLoginView();
				} else {
					System.out.println("재고가 없습니다. 죄송합니다.");
					BookMain();
				}

			} else {
				System.out.println("해당 책이 존재하지 않습니다. 다시 확인해주세요. ");
				BookMain();
			}
			...
			if (choice == 'Y') {
				...
				if (userChoice == 'Y') {
					...
					try {
						...
						// 반납일자 초과
						if (difference > 0) {
							System.out.println("반납일보다 '" + difference + "'일 초과되었습니다. ");
					...
					// 기록 삭제
					int result = dao.deleteBookedBook(serialNumber);
