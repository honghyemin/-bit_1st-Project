	...
	public void LoginView() {
		...
		while (true) {
			...
			if (loginChoice == 1) {
				...
				while (loginChoice < 1 || loginChoice > 5) {
					System.out.println("번호를 잘못 입력했습니다. 다시 입력 해주세요.");
					System.out.print("▶");
...
//		String id = sc.next();
//		
//		if(login.ididid(id) != null) {
//			MemberDTO member = new MemberDTO(id, "", "");
//			login.ididid(member);
//		}
//	}
		...
		while (true) {
			...
			if (login.checkId(id)) {
				...
				if (login.checkPw(id, pw)) {
					System.out.print(id + "님이 로그인 하셨습니다.");
					System.out.println(" 화면으로 이동합니다.");
					ilt.InsertLoginView();
					try {
						Thread.sleep(3000);
						LibraryMain.main(null);
...
//						
//						if(id==" ") {
//						System.out.println("아이디는 반드시 입력해주세요!");
//					}
					}
			...
			if (choiceId == 'Y') {
				...
				case 1:
					...
					if (login.checkPhone(phone) != null) {
						...
						System.out.println(" 3초 후 로그인 창으로 이동합니다. ");
						try {
							Thread.sleep(3000);
							LoginView();
						} catch (InterruptedException e) {
